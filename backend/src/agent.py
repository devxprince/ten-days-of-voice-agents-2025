...
class Assistant(Agent):
    def __init__(self) -> None:
        super().__init__(
            instructions="""
You are a friendly coffee shop barista for a specialty coffee brand called "Prince's Roastery".
...
async def entrypoint(ctx: JobContext):
    ...
    session = AgentSession(
        # STT: convert user speech → text
        stt=deepgram.STT(model="nova-3"),
        # LLM: brain of the agent
        llm=google.LLM(
            model="gemini-2.5-flash",
        ),
        # TTS: convert agent text → speech
        tts=murf.TTS(
            voice="en-US-matthew",
            style="Conversation",
        ...
        # allow preemptive response generation
    ...
    await session.start(
        ...
        room_input_options=RoomInputOptions(
            # Telephony-style noise cancellation
